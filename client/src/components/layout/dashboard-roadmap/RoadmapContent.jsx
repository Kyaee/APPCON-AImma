...
const RoadmapContent = ({ lessons = [], currentCourse, onCourseChange, isSidebarExpanded }) => {
  ...
  const boxPositions = useMemo(() => {
    ...
    const courseSeed = currentCourse.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    ...
    for (let i = 1; i < lessons.length; i++) {
      ...
      const verticalGap = MIN_VERTICAL_GAP + Math.floor(seed2 * VERTICAL_VARIATION);
      
      // Calculate vertical position based on previous box
      const prevBox = positions[i - 1];
      const verticalPosition = prevBox.verticalPosition + prevBox.boxSize + verticalGap;
  ...
  return (
    <div className="relative w-full mt-10">
      ...
      <div className="relative w-full mx-auto">
        ...
        <div className="fixed top-1/2 z-20 transform -translate-y-1/2" style={{ left: leftButtonPosition }}>
          <button 
        ...
        <div className="fixed top-1/2 z-20 transform -translate-y-1/2" style={{ right: rightButtonPosition }}>
          <button 
            className="bg-white p-3 rounded-lg shadow-md hover:bg-gray-100 border-2 border-black"
        ...
        <div className="relative z-0 mx-auto" style={{ width: '80%' }}>
          {lessons.map((lesson, index) => {
            ...
            const prevBoxCenterY = prevBox.verticalPosition + (prevBox.boxSize / 2);
            
            const currentBoxCenterX = `${currentBox.horizontalPosition}%`;
            const currentBoxCenterY = currentBox.verticalPosition + (currentBox.boxSize / 2);
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    <Play className={`${index === 0 ? 'w-14 h-14' : 'w-12 h-12'} text-[#6D4C41] fill-[#6D4C41]`} />
                  </Link>
                </div>
              </div>
            );
        ...
        <div style={{ 
          height: `${boxPositions.length > 0 ? 
            boxPositions[boxPositions.length - 1].verticalPosition + 
            boxPositions[boxPositions.length - 1].boxSize + 100 : 0}px` 
        }} />
      </div>
    </div>
  );
