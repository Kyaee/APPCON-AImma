...
const StreakPanel = ({ 
  days = 5,
  dailyStatus = ['completed', 'missed', 'future', 'future', 'future', 'future', 'future']
}) => {
          ...
          <div key={index} className="flex flex-col items-center">
            <div 
              className={cn(
                ...
                status === 'completed' && "bg-green-100 text-green-600 border-2 border-green-600",
                status === 'missed' && "bg-red-100 text-red-600 border-2 border-red-600",
                status === 'future' && "bg-gray-100 text-gray-400 border-2 border-gray-300"
              )}
            >
              {status === 'completed' && <Check className="w-4 h-4" />}
              {status === 'missed' && <X className="w-4 h-4" />}
            </div>
            <span className="text-xs mt-1 font-medium text-gray-600">{daysOfWeek[index]}</span>
          </div>
        ))}
      </div>
    </div>
