      ...
      {isOpen && (
        <div className="animate-text-fade-short absolute top-10 right-10 border border-foreground dark:border-dark-mode-highlight bg-background dark:bg-dark-inner-bg p-5 rounded-lg shadow-lg w-72 min-w-[260px] max-w-xs z-50 h-72 overflow-y-auto">
          ...
          {notifications.map((data)=> 
            ...
            >
              <span className="px-2 py-1 bg-neutral-200 dark:bg-dark-mode-highlight text-xs rounded-full group-hover:bg-accent dark:group-hover:bg-dark-mode-bg transition-colors">{new Date(data.created).toLocaleDateString('en-US', {
                weekday: 'long',
                year: 'numeric',
                month: 'long',
                day: 'numeric',
