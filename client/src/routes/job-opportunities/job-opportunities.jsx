...
export default function JobOpportunities() {
  ...
  return (
    <div className="relative w-full min-h-screen overflow-x-hidden">
      ...
      <div className="fixed top-8 right-15 flex items-center gap-4 z-10">
        <div className="flex items-center gap-4">
          <StatsDisplay type="heart" value="10" />
          <StatsDisplay type="gem" value="500" />
        </div>
        <div className="flex items-center gap-4">
          <IconButton type="notification" onClick={() => console.log('Notification clicked')} />
          <IconButton type="settings" onClick={() => console.log('Settings clicked')} />
        </div>
      </div>
      ...
      {isNewUser ? (
        <div className="w-full min-h-screen flex flex-col justify-center items-center pt-20">
          <div className="flex flex-col items-center justify-center">
            <img src={wavingCappy} alt="Welcome" className="w-auto h-auto object-contain mt-[60px]" />
            <span className="text-[#4C4C4C] text-[48px] font-bold mt-[10px]">Gather Job Opportunities</span>
            <span className="text-black text-[20px] mt-[10px]">Before you start,</span>
            <button
              onClick={handleStartClick}
              onMouseEnter={() => setIsHovered(true)}
              onMouseLeave={() => setIsHovered(false)}
              className={`mt-5 w-full max-w-[561px] h-[70px] rounded-[20px] border border-black
        ...
        <div className="w-full min-h-screen pt-32 px-4 md:px-[10%] lg:px-[15%] xl:px-[19%]">
          <h1 className="text-[#4C4C4C] text-5xl font-extrabold mb-4">Personalized for You</h1>
